...
public:
	...
	// with identifier
	node(const unsigned int idx_) { idx = idx_; }

	/*******************/
	/* member function */
	/*******************/

	void update_idx(const unsigned int idx_) { idx = idx_; }
};
...
private:
	...
	void split(node *n, const std::vector<unsigned int>& dataset_sub) {
		...
		if (dataset_sub.size() > capacity) {
			...
			// update node
			n->update_idx(dataset_sub[idx]);

			// compute distance mean
			std::vector<float> distances;
			distances.resize(dataset_sub.size());

			for (unsigned int i = 0; i < dataset_sub.size(); ++i) distances[i] = compute_distance(dataset[n->idx], dataset[dataset_sub[i]]);
...
public:
	...
	void build() {
		...
		for (unsigned int i = 0; i < dataset.size(); ++i) idx_set.push_back(i);
		...
		time_vptree_build = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
		std::cout << " vp-tree build time: " << time_vptree_build / 1000 << "[sec]\n\n";
	}

	// kNN search
	void kNN_search(data &d, const unsigned int idx) {
			...
			if (n->idx == -1) {
				...
				for (unsigned int i = 0; i < n->leaf.size(); ++i) {
                    ...
                    if (idx != n->leaf[i]) {
						...
						d.update_knn(distance, dataset[n->leaf[i]].identifier);
					}
				}
			}
			else {
				...
				float distance = compute_distance(d, dataset[n->idx]);
				if (n->idx != (int)idx) d.update_knn(distance, dataset[n->idx].identifier);

				// children check
				if (distance + d.knn_distance >= n->distance_mean) queue.push_back(n->right);
				if (distance - d.knn_distance <= n->distance_mean) queue.push_back(n->left);
			}
		}
	}
